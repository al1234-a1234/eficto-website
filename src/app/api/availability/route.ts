...
import { countAvailableTables, toRiyadhISOString } from "@/lib/reservationLogic";
import { isValidPartySize, isWithinOperatingHours } from "@/lib/validate";
...
export async function GET(request: Request) {
  ...
  if (!date || !time || !isValidPartySize(partySize)) {
  ...
  try {
    ...
    const tablesFree = await countAvailableTables(supabase, reservationTimeISO, partySize);
